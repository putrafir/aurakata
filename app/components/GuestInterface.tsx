...
import { Mic, Send, Smile, Heart, Angry, Clock, HelpCircle, Volume2, MessageCircle } from 'lucide-react';
...
export function GuestInterface({ messages, onSendMessage }: GuestInterfaceProps) {
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50 max-w-2xl mx-auto shadow-sm border-x border-slate-200">
      ...
      <header className="p-4 bg-white border-b border-slate-200 flex items-center justify-between sticky top-0 z-20">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] font-bold text-sky-500 uppercase tracking-widest">Terhubung ke Host</p>
          </div>
        </div>
      </header>

      {/* Chat Area (Sama seperti Host) */}
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-6 scroll-smooth pb-32">
        <AnimatePresence initial={false}>
          {messages.map((msg) => (
            <motion.div
              key={msg.id}
              initial={{ opacity: 0, x: msg.sender === 'guest' ? 50 : -50, scale: 0.8 }}
              animate={{ opacity: 1, x: 0, scale: 1 }}
              className={`flex flex-col ${msg.sender === 'guest' ? 'items-end' : 'items-start'}`}
            >
              <div
                className={`max-w-[85%] px-6 py-4 rounded-[32px] shadow-sm relative ${msg.sender === 'guest'
              ...
              >
                ...
                <DynamicText
                  ...
                  className={`text-lg ${msg.sender === 'guest' ? 'text-white' : ''}`}
                />

                {/* Icon Emosi Lawan Bicara */}
                {msg.sentiment && (
                  <div className="absolute -top-2 -right-2 w-6 h-6 bg-white rounded-full flex items-center justify-center shadow-md border border-slate-100">
                    {React.createElement(EMOTIONS.find(e => e.label === msg.sentiment)?.icon || Smile, {
                      size: 12,
                      className: msg.sender === 'guest' ? 'text-sky-500' : 'text-emerald-500'
                    })}
                  </div>
                )}
              </div>
              <span className="text-[9px] font-black text-slate-400 mt-2 uppercase tracking-widest px-2">
                {msg.sender === 'guest' ? 'Suara Anda' : 'Balasan Host (Tuli)'}
              </span>
            </motion.div>
          ))}
        </AnimatePresence>
      ...
      {/* Area Kontrol Bawah (Sama seperti Host + Mic Raksasa) */}
