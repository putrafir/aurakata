...
export function GuestInterface({ messages, onSendMessage, currentGuestName, onExit }: GuestInterfaceProps) {
  ...
  React.useEffect(() => {
    if (isListening) return; // Jangan tembak API teks biasa kalau lagi pakai mic
  ...
  const handleMicDown = async (e: React.PointerEvent) => {
    ...
    try {
      ...
      console.error("Gagal memulai pengukuran volume:", err);
      try { recognitionRef.current.start(); } catch (e) { } // Fallback
    }
  };

  const handleMicUp = async (e: React.PointerEvent) => {
    e.preventDefault();
    setIsListening(false);
    
    // Matikan pengukuran volume dan bersihkan microphone stream
    ...
    // Panggil API Hibrida (Volume + NLP) jika ada teks
    if (inputText.trim().length > 0) {
      const finalCompleteSentence = inputText.trim();
